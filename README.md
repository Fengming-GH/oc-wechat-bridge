...
|:-:|:---------|:------------|:----|:----------|
| 1 | `@fengming-gh/oc-forward` | [oc-forward](https://github.com/Fengming-GH/oc-forward) | v1.0.0 | 跨会话消息转发：`！` 指令，不阻塞源 AI |
| 2 | `@fengming-gh/oc-auto-continue` | [oc-auto-continue](https://github.com/Fengming-GH/oc-auto-continue) | v1.0.0 | 自动续命：terminated/API 错误后自动恢复 + 压缩后重读规则 |
| 3 | `@fengming-gh/oc-taskid-tracking` | [oc-taskid-tracking](https://github.com/Fengming-GH/oc-taskid-tracking) | v1.0.0 | 让子AI 带着上下文连续工作 |
...
```bash
# 克隆所有插件
git clone https://github.com/Fengming-GH/oc-forward.git
git clone https://github.com/Fengming-GH/oc-auto-continue.git
git clone https://github.com/Fengming-GH/oc-taskid-tracking.git

# 每个插件都是独立的 TypeScript 项目，结构统一：
# oc-xxx/
# ├── src/index.ts    # 插件源码
...
```bash
# 1. GitHub 创建仓库（Public + MIT + README + Node .gitignore）
# 2. GitHub Personal Access Token（classic，勾 repo 权限）
# 3. 本地配置
...
**如果远程已有文件导致冲突：** 使用 `git push -f -u origin main` 强制覆盖（仅限新仓库）。
...
# 2. 本地登录
npm login
# 输入用户名、密码、邮箱、OTP（如开启二步验证）

# 3. 检查 package.json 配置
#    - name: "@作用域/包名"
#    - publishConfig.access: "public"（作用域包必须）
#    - version: 语义化版本号
...
**作用域包说明：** `@fengming-gh/oc-forward` 是作用域包（scoped package），必须加 `--access public` 发布，否则 npm 默认私有。
